...
public class WinValidator {
     ...
     * 5. 抢金：开金后，在“抢金窗口”内只要玩家已经处于听牌状态，
     *    且满足“16 张暗牌（不含花）进一张金牌”的进张方式，即视为一种
     *    特殊胡牌方式，不再强制 17 张牌按普通牌型（3n+2 面子+将）拆完。
     * 6. 三头金：手里有3张金
     */
    public static boolean canWin(List<Tile> handTiles, Tile goldTile, boolean isQiangJin) {
        ...
        // 抢金判断（开金后：满足“16 张暗牌时进一张金即可和”）
        // 注意：三头金优先级更高，因此必须在三头金之后判断。
        // 说明：
        //   - “是否属于抢金进张”由 GameEngine.isQiangJinForCurrentDraw 负责；
        //   - 本方法只在 isQiangJin=true 且张数合法（3n+2，实际为 17 张）时，
        //     直接视为可以胡牌，不再要求 17 张牌本身也能按普通牌型拆完。
        ...
        if (isQiangJin) {
            // 此处不再调用 canWinWithCounts 做二次牌型校验：
            // 只要 GameEngine 已经判定本次摸牌属于“抢金进张”，
            // 且当前张数满足 3n+2（上方已校验），直接视为可以胡牌。
            //
            // 兜底：抢金场景下应当是 17 张牌，这里仅保守验证张数。
            return totalTileCount == 17;
        }
